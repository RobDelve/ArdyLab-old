...
<#
    .Synopsis
    ArdyLab DSC Resource to control the build of required Virtual Machine nodes
...
{    
  ...
  node localhost
  {
    ...
    })
    {
      ...
      $GlobaladminName = ($AllNodes.where{
          $_.NodeName -eq '*'
      }).AdminName
      $GlobaladminPassword = ($AllNodes.where{
          $_.NodeName -eq '*'
      }).AdminPassword
      ...
      if ($vm.role -contains 'FileServer')
      {
        ...
        {
          # create a DSC Resource to add-vmharddiskdrive....
          # Add-VMHardDiskDrive -VMName $vm.NodeName 
          LabAddVHD "$($vhd.ShareName)"
          {
            NodeName = $vm.NodeName
            Path = $vhd.path
