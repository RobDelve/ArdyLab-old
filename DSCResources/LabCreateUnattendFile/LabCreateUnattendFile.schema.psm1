...
configuration LabCreateUnattendFile {
  ...
  Script CreateUnattendFile
  {
    ...
    SetScript = 
    {
      [xml]$unattendFile = Get-Content -Path ( Convert-Path -Path $using:ConfigurationData.LabConfig.FilePaths.Unattend.TemplateFile )
      ...
      $customUnattendFile = "$(Convert-Path -Path $using:ConfigurationData.LabConfig.FilePaths.Unattend.OutputFolder)\$($using:NodeName)_unattend.xml"
